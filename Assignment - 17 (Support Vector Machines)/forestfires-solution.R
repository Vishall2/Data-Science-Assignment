...
ggplot(forest1) + geom_bar(aes(temp), binwidth = 0.50, fill = "red") + theme_classic()
...
#Here the Accuracy of the Model is 76.62%
...
model2 #Training Error - 0.258953
...
model3 #Training Error - 0.115702 
...
model8 #Training Error - 0.057851 
pred8 <- predict(model8, test1[-11])
tab8 <- table(test1[,11], pred8)
equals8 <- test1[,11] == pred8
prop.table(table(equals8))
#Here the Accuracy of the Model is 67.53%

#The Laplacedot Kernel Model has the Least Training Error with a Accuracy of 69.94%
